...
load_numdeath_2 <- function(numdeath.file.path = "/censoc/data/numident/numident_files_with_original_varnames/numident_death_records_complete.csv") {
  ...
  numdeath[,"byear_death_file" := as.numeric(substr(NUMI_DOB_DTH_1, 5, 8))]
  numdeath[,"dyear" := as.numeric(substr(NUMI_DOD_DTH_1, 5, 8))]

  ## birth and death month
  numdeath[,"bmonth_death_file" := as.numeric(substr(NUMI_DOB_DTH_1, 1, 2))]
  numdeath[,"dmonth" := as.numeric(substr(NUMI_DOD_DTH_1, 1, 2))]

  ## birth and death day
  numdeath[,"bday_death_file" := as.numeric(substr(NUMI_DOB_DTH_1, 3, 4))]
  ...
  numdeath[, c("NUMI_DOB_DTH_1","NUMI_DOD_DTH_1", "NUMI_NH_NAME_FIRST_DTH_1",
               "NUMI_NH_NAME_LAST_DTH_1", "NUMI_SSN", "NUMI_SEX_DTH_1", "NUMI_ZIP_RESIDENCE_1"):=NULL]
