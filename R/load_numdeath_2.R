...
load_numdeath <- function(numdeath.file.path = "/censoc/data/numident/numident_files_with_original_varnames/numident_death_records_complete.csv") {

  all_cols_to_keep <- c("NUMI_SSN", "NUMI_NH_NAME_FIRST", "NUMI_NH_NAME_LAST", "NUMI_SEX", "NUMI_DOB", "NUMI_DOD_DTH_1", "NUMI_ZIP_RESIDENCE_1")

  numdeath <- fread(path numdeath.file.path select=all_cols_to_keep, colClasses = list(character= 'NUMI_SSN', 'NUMI_ZIP_RESIDENCE_1', 'NUMI_DOB', 'NUMI_DOD_DTH_1')))
