...
censoc <- fread("/data/josh/CenSoc/censoc_data/censoc_linked_to_census/v2.1/censoc_dmf_v2.1_linked.csv")# This is the path in the demo file, now using v2.1. Path below didn't go anywhere
...
By excluding the right tail of the distribution, the right truncation generally creates a downward bias on the estimated effects of any covariates. The effect of these coefficients would be even larger, and the appropriate regression coefficient adjustment factors can be found through simulating a Gompertz mortality schedule. For more information on regression coefficient adjustment factors, see our [BUNMD Working Paper](https://censoc.berkeley.edu/wp-content/uploads/2022/02/bunmd_paper_feb10_2022.pdf). 
