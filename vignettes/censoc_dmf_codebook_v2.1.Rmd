...
## read in censoc_dmf_v2 data file
censoc_dmf_v2 <- read_csv("/data/josh/CenSoc/data_release_v2/censoc_dmf_v2.csv")
```

```{r, include = F}
byear_plot <- censoc_dmf_v2 %>%
    group_by(byear) %>%
    summarise(n = n()) %>%
...
bmonth_tabulated <- knitr::kable(censoc_dmf_v2 %>%
    filter(bmonth != 0) %>% 
    group_by(bmonth) %>%
    tally() %>%
...
```{r, include = F}
dmonth_tabulated <- knitr::kable(censoc_dmf_v2 %>%
    group_by(dmonth) %>%
    tally() %>%
    mutate(freq = signif(n*100 / sum(n), 2)) %>%
    mutate(label = c("January", "February", "March", "April", "May", "June", "July", "August", "September", "October", "November", "December")) %>%
...
```{r, include = F}
link_abe_exact_conservative_tabulated <- knitr::kable(censoc_dmf_v2 %>%
...
```{r, include = F}
weights_tabulated <- censoc_dmf_v2 %>%
  filter(!is.na(weight)) %>% 
...
```{r, include = F}
weights_conservtive_tabulated <- censoc_dmf_v2 %>%
...
weight_conservative.plot <- censoc_dmf_v2 %>%
  filter(link_abe_exact_conservative == 1) %>% 
  filter(!is.na(weight_conservative)) %>% 
  ggplot() +
