...
race_match_rate <- census_1940_hh_vars_recode %>% 
  select(histid, link_abe_exact_standard, link_abe_exact_conservative_dummy, race_recode) %>%
  pivot_longer(-c(histid, link_abe_exact_standard, link_abe_exact_conservative_dummy)) %>% 
  group_by(name, value) %>%
  summarize(Standard = mean(!is.na(link_abe_exact_standard)),
...
education_match_rate <- census_1940_hh_vars_recode %>% 
  select(histid, link_abe_exact_standard, link_abe_exact_conservative_dummy,  educ_level) %>%
  pivot_longer(-c(histid, link_abe_exact_standard, link_abe_exact_conservative_dummy)) %>% 
  group_by(name, value) %>%
    summarize(Standard = mean(!is.na(link_abe_exact_standard)),
            Conservative = mean(link_abe_exact_conservative_dummy)) %>% 
...
region_match_rate <- census_1940_hh_vars_recode %>% 
  select(histid, link_abe_exact_standard, link_abe_exact_conservative_dummy, region_string) %>%
  pivot_longer(-c(histid, link_abe_exact_standard, link_abe_exact_conservative_dummy)) %>% 
  group_by(name, value) %>%
    summarize(Standard = mean(!is.na(link_abe_exact_standard)),
            Conservative = mean(link_abe_exact_conservative_dummy)) %>% 
...
, filename = here("vignettes/assess_match_quality/figs/dmf_match_rate_subgroups.pdf"), height = 3, width = 14)
```
