...
---
#This rmd recodes grade_codes, branch_codes,source, branch_alpha, grade_alpha,
# term_or_enlistment, and component
...
```{r}
path_to_temp_data   <-  "/data/josh/CenSoc/censoc_data/enlistment-records-public/code/awenlistmentcode/temp4.csv"
out_path <- "/data/josh/CenSoc/censoc_data/enlistment-records-public/code/awenlistmentcode"
grade_codes_path <- "/data/josh/CenSoc/censoc_data/enlistment-records-public/code/awenlistmentcode/titlesnicknames/cl_2072.csv"
...
#read in csv of NARA AAD grade codes
```{r}
grade_codes <- read_csv(grade_codes_path)
```
...
#Read in csv with NARA AAD branch codes
``` {r}
branch_codes <- read_csv(branch_codes_path)

```

#maybe use ranges for this formula depending on what other numbers mean
...
#RECODING source
#read in csv of NARA AAD source
```{r}
source_codes <- read_csv(source_path)
```
#Count of source
...
#RECODING branch_alpha
#read in csv of NARA AAD source
```{r}
branch_alpha_codes <- read_csv(branch_alpha_path)
```
#Count of branch_alpha
...
#Create function to recode branch_alpha 
```{r}
recode_branch_alpha_enlistment <- function(df){
  
  ## recode grade variable
  df <- df %>% 
    mutate(branch_alpha_recode = case_when(
      branch_alpha == "0" ~ 0,
      branch_alpha == "6" ~ 6,
      branch_alpha == "2" ~ 2,
      branch_alpha == "1" ~ 1,
...
#read in csv of NARA AAD source
```{r}
grade_alpha_codes <- read_csv(grade_alpha_path)
```
#Count of grade_alpha
```{r}
data_combined %>% 
  count(grade_alpha) %>%
  arrange(desc(n))
...
#read in csv of NARA AAD term_or_enlistment
```{r}
term_or_enlistment_codes <- read_csv(term_or_enlistment_path)
```
#Count of term_or_enlistment
```{r}
data_combined %>% 
  count(term_or_enlistment) 
```
...
#RECODE component
#read in csv of NARA AAD component
```{r}
component_codes <- read_csv(component_path)
```
#Count of component
...
component_recode <- recode_component_enlistment(df = data_combined) %>% 
  select(component, component_recode)
```

#Add columns with recoded data to data_combined
```{r}
data_combined <- cbind(data_combined, source_recode$source_recode, grade_recode$grade_code_recode,term_or_enlistment_recode$term_or_enlistment_recode, branch_recode$branch_code_recode, component_recode$component_recode)
```
