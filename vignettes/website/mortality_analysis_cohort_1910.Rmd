...
IPUMS variables often have value labels, which is text that corresponds to numeric codes (e.g., the `SEX` variable has value labels: 1 = "Male", 2 = "Female").

The `IPUMSR` package includes a helpful functions for assigning values labels (see the [ipumsr website](http://tech.popdata.org/ipumsr/index.html) for more information and tutorials). In this vignette, for simplicity, we'll manually code our value labels. 
...
Exploratory data analysis is an important part of the modeling process. Exploratory data analysis can help us understand the main characteristics of the data and their relationship with our dependent variable, age at death. Here, we’ll briefly explore some associations between our independent and dependent variables.
...
In this case, we're only looking at the cohort of 1910, so we don't need to include a fixed effect term for year of birth. 
