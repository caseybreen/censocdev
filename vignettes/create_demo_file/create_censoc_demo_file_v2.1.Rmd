...
numidentdata <- fread("/data/josh/CenSoc/censoc_data/censoc_linked_to_census/v2.1/censoc_numident_v2.1_linked.csv")
...
dmfdata <- fread("/data/josh/CenSoc/censoc_data/censoc_linked_to_census/v2.1/censoc_dmf_v2.1_linked.csv")
...
#Create separate data.frame for statefip values and add Alaska and Hawaii 
  numident_dmf %>% 
    ...
    count(statefip) %>% 
    add_row(statefip= c(2,15), n = c(0,0)) %>% arrange(statefip)->statenum
...
state_new<-tibble(state.name) %>% 
  ...
  arrange(state.name) %>% 
  mutate(statefip = as_factor(statenum$statefip))
...
## Write out csv file 
write_csv(demo_file_dmf, "/data/josh/CenSoc/data_release_demo_v2.1/censoc_dmf_demo_v2.1.csv")
write_csv(demo_file_numident, "/data/josh/CenSoc/data_release_demo_v2.1/censoc_numident_demo_v2.1.csv")
```
