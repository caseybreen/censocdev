...
**Description**: id is an 11-digit string generated to uniquely identify each records in the dataset.
...
**Description**: byear is a numeric variable reporting a person’s year of birth, as recorded in the Numident death records.
...
toe_tabulated <- knitr::kable(data_records %>%
    group_by(term_of_enlistment_enlistment) %>%
     ...
     select(term_of_enlistment_enlistment, `label (abbreviated)` = label, n, `freq %` = freq))
```
...
**Description**: HISTID is a unique individual-level identifier assigned to individuals in IPUMS-USA 1940 Census data. This variable is available for individuals where a link between army enlistment records and the 1940 Census has been established.
