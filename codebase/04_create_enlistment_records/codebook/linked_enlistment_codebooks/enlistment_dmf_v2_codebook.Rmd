...
|   \hyperlink{page.6}{6}  |\hyperlink{page.6}{dyear\_DMF}            |Year of birth        |
|   \hyperlink{page.7}{7}  | \hyperlink{page.7}{dmonth\_DMF}          |Month of birth     |
...
**Summary:** The CenSoc Enlistment-DMF files links 1,854,783 men from World War II era (circa 1938-1947) army enlistment records to the social security Death Master File (DMF). The suffix on each variable inidicates if the variable comes from DMF mortality records or army enlistment records. For enlistment records that have been independently linked to the 1940 census, we include the HISTID identifier variable that researchers may use to merge this data set with IPUMS 1940 census data. Matches between datasets were established using the conservative variant of the ABE method developed by Abramitzky, Boustan, and Eriksson ([2012](https://ranabr.people.stanford.edu/sites/g/files/sbiybj5391/f/abe_ageofmassmigration.pdf), [2014](https://ranabr.people.stanford.edu/sites/g/files/sbiybj5391/f/abe_assimilation_1.pdf), [2017](https://ranabr.people.stanford.edu/sites/g/files/sbiybj5391/f/return-migrants.pdf)).
...
**Description**: id is a 9-digit string that uniquely identifies each records in the dataset.
...
**Description:** bmonth_ reports a person's month of birth, as recorded in the DMF.  
...
**Description**: dyear reports a person's year of death, as recorded in the DMF.    
...
**Description**: dmonth reports a person's month of death, as recorded in the DMF.  
...
**Description**: death_age reports a person's age at death in years, calculated using the birth and death information recorded in the DMF.  
...
**note**: The matching algorithm used allows for up to two years of flexibility on birth year, so birth year in the DMF may disagree with birth year in enlistment records. Birth years are different by one year in 7.8\% of records, and different by two years in 4.3\% of records. Both birth year variables are published in the data set.
...
toe_tabulated <- knitr::kable(data_records %>%
    group_by(term_of_enlistment_enlistment) %>%
     ...
     select(term_of_enlistment_enlistment, label, n, `freq %` = freq))
```
...
Refer to the [\textcolor{blue}{National Archives' technical documentation on the Amy Serial Number Electronic File}](https://catalog.archives.gov/id/1263923) for further information. For enlistees who enlisted starting in March 1943, we have created an additional variable called **weight_or_AGCT_enlistment**.
...
Refer to the [\textcolor{blue}{National Archives' technical documentation on the Amy Serial Number Electronic File}](https://catalog.archives.gov/OpaAPI/media/1263923/content/arcmedia/electronic-records/rg-064/asnf/100.1ND_NC.pdf?download=false) for further information. For enlistees prior to January 1943, we used the information recorded in this field to create the **weight_before_march_1943_enlistment** variable.
...
component_tabulated <- knitr::kable(data_records %>%
    group_by(component_enlistment) %>%
     ...
     select(component_enlistment, label, n, `freq %` = freq))
...
source_tabulated <- kableExtra::kable(data_records %>%
    group_by(source_enlistment) %>%
     ...
     select(source_enlistment, label, n, `freq %` = freq))
...
**Description**: HISTID is a unique individual-level identifier assigned to indivuals in IPUMS-USA 1940 census data. This variable is availible for individuals where a link between army enlistment records and the 1940 census has been established.
