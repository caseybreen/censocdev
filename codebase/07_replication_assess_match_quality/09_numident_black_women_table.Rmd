---
title: "CenSoc Numident: Black Women Characteristics"
author: "Kathy Perez"
date: "1/26/2022"
---


Summary: Characteristics of Black women in Numident data
...
```{r}
census_1940_hh_vars_recode_baw  <- census_1940_hh_vars %>% 
...
#Generate table
```{r}
## tabulations for women in 1940 census 
census_characteristics_baw <- census_1940_hh_vars_recode_baw %>% 
...
## tabulations for women in CenSoc-Numident standard 
numident_characteristics_standard_baw <- census_1940_hh_vars_recode_baw %>% 
...
## tabulations for women in CenSoc-Numident conservative 
numident_characteristics_conservative_baw <- census_1940_hh_vars_recode_baw %>% 
...
combined_characteristics_baw <- census_characteristics_baw %>% 
  inner_join(numident_characteristics_standard_baw, by = c("name", "value")) %>% 
  inner_join(numident_characteristics_conservative_baw, by = c("name", "value")) %>% 
...
table_s3_baw <- gt(data = combined_characteristics_baw) %>% 
  tab_spanner(
    label = "General Pop",
    columns = vars(
      n_gen, prop_gen)) %>% 
...
```{r}
table_s3_baw %>%
  gtsave("numident_characteristics_black_women_table.tex", path = here("vignettes/assess_match_quality/figs/"))
```
